#!/usr/bin/env bash
# Historical V3 R1 extension workflow. Do not use for V4 training.
set -euo pipefail
...
# 训练 E1/E2（baseline + NEFTune），统一使用当前 v3 运行时提示词评估
# run_variant 内置 is_complete 幂等检查，已完成的 variant 会自动跳过
for variant in e1 e2; do run_variant "$variant"; done

gpu=$(wait_for_gpu)
CURRENT_STAGE=baseline_prompt_v3_seed_${SEED}
write_state evaluating "variants=e1,e2 gpu=$gpu prompt=v3 scope=baseline"
log "baseline_prompt_v3_start seed=$SEED gpu=$gpu variants=e1,e2"
bash "$PROJECT/scripts/lab-evaluate-kisaki-r1-seed.sh" "$SEED" "$gpu" prompt-v3-baseline e1,e2
for variant in e3 e4 e5; do run_variant "$variant"; done
gpu=$(wait_for_gpu)
CURRENT_STAGE=evaluation_seed_${SEED}
write_state evaluating "variants=e1,e2,e3,e4,e5 gpu=$gpu prompt=v3"
bash "$PROJECT/scripts/lab-evaluate-kisaki-r1-seed.sh" "$SEED" "$gpu"
write_state pilot_complete 'R1 E1-E5 prompt-v3 evaluation complete; independent blind review pending'
