...
async def _execute_sql(query: str, params: Optional[dict] = None):
    """Run synchronous database work without blocking FastAPI's event loop."""
...
async def _create_experiment_record(
    ...
    try:
        ...
        logger.exception("failed to create experiment=%s", experiment_id)
        raise HTTPException(
            status_code=503,
            detail="无法创建实验记录，请稍后重试",
...
async def list_experiments(experiment_type: Optional[str] = None,
    ...
    try:
        ...
        logger.exception("列出实验失败")
        raise HTTPException(status_code=503, detail="实验记录暂时不可用") from e


@router.get("/api/experiments/{exp_id}")
async def get_experiment(exp_id: str, current_user: dict = Depends(get_current_admin)):
    """获取单个实验详情"""
    try:
        rows = await _execute_sql("SELECT * FROM experiment_runs WHERE id=:id", {"id": exp_id})
        if not rows:
...
async def start_lora_ablation(req: ExperimentStartRequest,
    ...
    await _create_experiment_record(
        exp_id,
        "lora_ablation",
        req.hypothesis or "LoRA vs DoRA vs RSLoRA ablation",
    )
    if req.mock:
        mock_results = {
            "mock": True,
...
async def start_rag_ablation(req: ExperimentStartRequest,
    ...
    await _create_experiment_record(
        exp_id,
        "rag_ablation",
        req.hypothesis or "vector vs BM25 vs hybrid vs hybrid+reranker",
    )
    try:
...
async def start_quantization_benchmark(req: ExperimentStartRequest,
    ...
    await _create_experiment_record(
        ...
        req.hypothesis or "FP16 vs AWQ vs NF4 vs INT8 comparison",
    )
    try:
        ...
        if not req.mock:
            error = "Quantization benchmark module is unavailable"
            try:
                await asyncio.to_thread(
                    db.execute_sql,
                    "UPDATE experiment_runs SET status='failed', completed_at=:ts, results=:r WHERE id=:id",
                ...
                logger.exception("failed to persist quantization import error experiment_id=%s", exp_id)
        ...
        logger.exception("量化基准实验失败 experiment_id=%s", exp_id)
        await _execute_sql(
            "UPDATE experiment_runs SET status='failed', completed_at=:ts, results=:r WHERE id=:id",
            {"ts": _now(), "r": json.dumps({"error": str(e)}), "id": exp_id},
        )
        raise HTTPException(status_code=500, detail="量化基准实验失败") from e


@router.get("/api/experiments/{exp_id}/report")
async def download_report(exp_id: str, current_user: dict = Depends(get_current_admin)):
    """获取实验报告（Markdown 格式）"""
    try:
        rows = await _execute_sql("SELECT * FROM experiment_runs WHERE id=:id", {"id": exp_id})
        if not rows:
        ...
        logger.exception("获取实验报告失败")
        raise HTTPException(status_code=503, detail="实验报告暂时不可用") from e
