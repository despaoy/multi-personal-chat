...
class AskRequest(BaseModel):
    ...
    maxTokens: int = Field(1024, ge=64, le=4096)
    topP: float = Field(0.9, ge=0.1, le=1.0)
    wantCitations: bool = True
    debug: bool = False


class AskResponse(BaseModel):
    """知识问答响应：非流式与流式最终结构语义一致。"""
...
class KnowledgeSearchRequest(BaseModel):
    ...
    knowledgeBaseName: str | None = None  # 按知识库名称过滤检索结果
...
class SaveDialoguesRequest(BaseModel):
    ...
    style: str | None = None
    dialogues: list
    turn_stats: dict[str, int] | None = None
    scene_stats: dict[str, int] | None = None
...
class EvalRunRequest(BaseModel):
    ...
    adapter_name: str | None = Field(default=None, max_length=256)
    ...
    max_prompts: int | None = Field(default=None, ge=1, le=50)
    mock: bool = False
...
class PreferencePairCreate(BaseModel):
    ...
    rubric: dict[str, float] | None = Field(default=None, max_length=20)
    annotator: str = Field(default="manual", min_length=1, max_length=100)
    metadata: dict[str, Any] | None = Field(default=None, max_length=100)
...
class PreferencePairUpdate(BaseModel):
    ...
    review_status: PreferenceReviewStatus | None = None
    rubric: dict[str, float] | None = Field(default=None, max_length=20)
    annotator: str | None = Field(default=None, min_length=1, max_length=100)
...
class RouterConfigUpdate(BaseModel):
    ...
    default_adapter: str | None = Field(default=None, min_length=1, max_length=128)
    mode: Literal["manual", "rule", "intent"] | None = None
    persona_adapters: dict[str, str] | None = Field(default=None, max_length=20)
    rag_confidence_threshold: float | None = Field(default=None, ge=0.0, le=1.0)
    persona_keywords: dict[str, list[str]] | None = Field(default=None, max_length=20)

    @field_validator("persona_adapters")
    @classmethod
    def validate_persona_adapters(cls, value: dict[str, str] | None):
        if value is None:
            return value
    ...
    @classmethod
    def validate_persona_keywords(cls, value: dict[str, list[str]] | None):
        if value is None:
            return value
...
class IntentSampleGenerateRequest(BaseModel):
    ...
    lora_name: str | None = Field(default=None, max_length=256)


class IntentTrainRequest(BaseModel):
    """Request for training the lightweight intent classifier."""

    kb_ids: list[int] | None = Field(default=None, max_length=8)
...
class FeedbackCreate(BaseModel):
    ...
    adapter_name: str | None = Field(default=None, max_length=256)
    kb_revision: str | None = Field(default=None, max_length=256)
...
class RetrievalEvalQuestionCreate(BaseModel):
    ...
    gold_answer: str | None = Field(default=None, max_length=10000)
