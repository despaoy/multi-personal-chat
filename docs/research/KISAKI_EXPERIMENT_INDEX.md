...
- R0V4：当前 1002 条训练数据（576 原作 + 150 既有构造 + 276 条 V4.1 五轮会话）和 70 条 validation；Gold v2.1 已批准为开发集，Gold v3 已冻结。
- 数据复审：Game Train 上下文质量复审尚未关闭；V4.1 的 276 条五轮会话（DeepSeek round06 4 条 + Codex 自动化批次 272 条）均已修订/批准并完成 Gold 污染复审。
- R1V4：等待 Game Train 复审完成并重新通过训练门禁，不得提前运行。
