...
> 当前执行入口：[月社妃 V4 人工审核与重训练](KISAKI_V4_HUMAN_REVIEW_AND_RETRAINING.md)。正式冻结训练输入仍为 V4；V4.1 增补、V5 候选和归档证据同时保留，不能把候选目录视为新的正式基线。

## 当前状态

- 2026-09-18 独立扩展：正史/反事实分支 MVP 和开发探针已加入，使用自编故事验证评测工具，不使用或修改冻结 Gold；当前无模型对照结果，不并入 R1V4 训练结论。见 [分支架构](../architecture/NARRATIVE_BRANCHES.md)。
...
3. 已保留的旧实验位于明确的 archive 目录，其他旧记录通过 Git 历史追溯；两者均不作为当前执行入口。
