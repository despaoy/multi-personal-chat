...
- V4 当前包含 926 条训练数据：522 条经最终上下文复审的原作对话、150 条既有构造数据，以及 254 条经审核保留的多轮用户模拟（3 条 DeepSeek + 251 条 Codex）。
...
我从 1,598 条原作直接台词建立可追踪语料，并重建对话边界、RAG held-out 和固定 split。现在 V4 有 926 条训练数据和 70 条独立验证数据；训练集由 522 条经上下文复审的原作样本、150 条既有构造样本和 254 条经审核保留的多轮用户模拟组成。Gold v2.1 已批准为开发集，Gold v3 的 150 条最终盲测已冻结；Game Train 复审与后续长度清理均有独立 provenance。
...
2. **数据，90 秒**：1,598 条来源语料、926 条 canonical 训练数据、70 条独立验证数据、人工审核与泄漏审计。
...
| V4 train | 当前 926 条 | 522 条原作、150 条构造、254 条审核多轮；已冻结 |
...
### 7.2 926 条 canonical 训练数据如何组成

- 522 条 `game_extraction_current_sft`：Game Train 上下文复审后保留，并隔离 validation 与 RAG evidence。
- 150 条既有构造数据：由此前 159 条复核后排除 9 条得到，已统一批准。
- 3 条 `deepseek_user_simulation_v41_reviewed` 与 251 条 `codex_user_simulation_v41_reviewed`：由增补快照经长度/技术相关性清理后保留，生成、审核和晋升链仍完整保存。
...
4. **为什么训练最终是 926 条？** V4.1 增补后曾达到 1002 条；Game Train 复审排除 54 条后为 948 条，长度清理再把 22 条代码主导或截断损伤记录移入技术辅助集，最终收敛为 926 条。
...
2. 展示 V4 canonical manifest：1,598 条来源、926 条训练数据、70 条 validation，以及已关闭的复审记录。
