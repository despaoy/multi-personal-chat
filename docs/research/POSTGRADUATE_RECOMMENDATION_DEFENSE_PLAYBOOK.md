...
- V4 当前包含 1002 条训练数据：576 条可回溯原作对话、150 条既有构造数据，以及 276 条经审核修订的 V4.1 五轮用户会话（4 条 DeepSeek round06 + 272 条 Codex 自动化批次）。
- 独立 validation 已审核并冻结 70 条。
- Gold v2.1 已批准为开发集，Gold v3 的 150 条最终盲测已审核并冻结；当前 blocker 是 Game Train 上下文质量复审。
...
我从 1,598 条原作直接台词建立可追踪语料，并重建对话边界、RAG held-out 和固定 split。现在 V4 有 1002 条训练数据和 70 条独立验证数据；训练集由 576 条原作样本、150 条既有构造样本和 276 条经审核晋升的五轮会话组成。Gold v2.1 已批准为开发集，Gold v3 的 150 条最终盲测已冻结；由于我又发现原作问答脱离剧情后可能不自然，当前主动恢复了 Game Train 上下文质量门禁。
...
2. **数据，90 秒**：1,598 条来源语料、1002 条 canonical 训练数据、70 条独立验证数据、人工审核与泄漏审计。
...
9. **下一步，30 秒**：完成 Game Train 上下文质量复审，重新通过门禁后运行 E1-E5 正式训练。
...
| V4 train | 当前 1002 条 | 576 条原作、150 条既有构造、276 条经审核晋升的五轮会话；整体复审中 |
...
构造数据没有冒充原作，并保留审核记录。canonical manifest 当前是 `frozen_under_reassessment`，Gold v3 已冻结，但最终 V4 仍等待 Game Train 上下文质量复审关闭。
...
4. **为什么训练从 730 增长到 1002 条？** 基础 730 条已经过连续对话配对、人物归属、人工排除和 validation 划分；V4.1 增补只以完整五轮会话为单位逐批审核晋升，不拆成重复前缀。
...
2. 展示 V4 canonical manifest：1,598 条来源、1002 条训练数据、70 条 validation，以及当前复审 blocker。
