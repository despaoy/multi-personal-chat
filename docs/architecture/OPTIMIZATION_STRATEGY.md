...
> 文档核对：2026-09-11。以下 P0/P1/P2 是实现与验收目标，不代表每项已通过生产测试。实际实现边界见 [当前状态](../CURRENT_STATE.md)。
...
- 幂等标识包含平台、适配器、会话类型、会话、发送者和消息 ID；无原生 ID 时使用事件级 ID，不能保证跨进程重新构造后仍相同。

## LoRA

- P0：后端 `LORA_PATH` 与 `VLLM_LORA_ROOT` 应映射到同一份适配器存储，容器路径可以不同；训练输出晋升为可加载 adapter 前须通过质量门禁。
...
- P0：通用知识库使用 `VECTOR_DB_PATH`，角色作品索引使用 `CHARACTER_RAG_INDEX_ROOT`；两者分别维护版本与来源，不混用索引。
...
- P1：通过 `traceId` 关联日志；已生成但未确认发送的重复事件复用回复并允许重试发送，已确认的重复事件避免重复更新。发送与确认不是原子事务，不承诺跨平台严格恰好一次。
- P2：分别上报网关可达性和平台活跃度，避免将空闲误判为断连。

## 高并发

当前 FastAPI 必须保持单进程 worker（`BACKEND_WORKERS=1`）；以下队列 worker 是进程内部的并发工作单元，不等于增加 Uvicorn 进程。
...
| 领域 | 自动化检查 | 生产验收 |
| --- | --- | --- |
| LoRA | adapter 扫描与激活契约测试 | 加载经批准的兼容 adapter、生成、回滚；当前月社妃无正式 adapter |
