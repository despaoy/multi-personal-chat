...
>
> 文档核对：2026-09-11；运行环境及实验结果日期保留原记录，当前工作区新增行为见 [当前状态](../CURRENT_STATE.md)。详细实验导航见 [月社妃实验总览](../research/KISAKI_EXPERIMENT_INDEX.md)。
...
旧 E1、E2、E2' Safety++ 和 E2'' RAG 不进入 V4 结论。为避免仓库内形成第二套入口，部分旧数据、脚本和结果保存在明确的 archive 目录，其他记录从 Git 历史追溯；不能把它们当作活动入口。
...
2026-09-18 新增实验性正史/反事实分支 MVP：Web 页面 `/narrative`，迁移 `009_narrative_branches`，开关 `NARRATIVE_BRANCHES_ENABLED` 默认关闭。分支不写入普通记忆，事实/历史/关系独立，提交使用 revision CAS。开发评测仅有自编故事和脚本化对照，没有真实模型结论；见 [分支架构与验证边界](../architecture/NARRATIVE_BRANCHES.md)。未改变现有训练冻结状态。
